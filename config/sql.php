<?php
  ...
  class Sql {
    ...
    public static function getUserNearStoreList($storeName,$lng,$lat,$radius,$scope,$offset=0,$pagesize=10) {
      if (empty($storeName)) {
        return "select store.*,store.store_id as _id,store_setting.* from store LEFT JOIN store_setting ON store.store_id = store_setting.store_id where  MBRContains(LineString(Point({$lat} + {$radius} / ( 111.1 / COS(RADIANS({$lng}))),{$lng} + {$radius} / 111.1),Point({$lat} - {$radius} / ( 111.1 / COS(RADIANS({$lng}))), {$lng} - {$radius} / 111.1 )), points) and status in(1) limit {$offset},{$pagesize}";
      }
